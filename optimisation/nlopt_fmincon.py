...
def run_nlopt(
    ...
    x0_real = np.array(x0_override if x0_override is not None else cfg["x0"], dtype=float)
    ...
    def objective(x_opt, grad):
        ...
        x_real = from_unit(x_opt) if scale_to_unit else np.array(x_opt, dtype=float)
    ...
    x_opt_real = from_unit(x_opt_found) if scale_to_unit else np.array(x_opt_found, dtype=float)
    if round_integers:
...
def multistart_nlopt(
    ...
    if verbose:
        ...
        for name, val in zip(cfg["overrides"], best_x):
            print(f"  {name}: {val:.6f}")
        print("Max annual energy:", best_energy)
